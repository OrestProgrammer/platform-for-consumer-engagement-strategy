...
knn = KNeighborsClassifier(n_neighbors=5, weights='distance', algorithm='auto', leaf_size=20, metric='euclidean', p=1)
...
disp_cm_val = mi.show_confusion_matrix(y_val, y_pred_val, knn)
disp_cm_test = mi.show_confusion_matrix(y_test, y_pred_test, knn)
...
lc_plt = mi.plot_learning_curve(knn, X_train_scaled, y_train)
lc_plt.show()

classes = np.unique(y_test)
y_test_bin = label_binarize(y_test, classes=classes)
roc_plt = mi.plot_roc_curve(knn, X_test_scaled, y_test_bin, classes)
roc_plt.show()